...
public class Loop {
	public static String[] loopKinds = { "ピアノ", "ギター" };
	public static String[] pianoTemplates = { "伸ばし" };
	public static String[] guitarTemplates = { "伸ばし" };

	public static final int iResolution = 480;

	public static ArrayList<MyNoteEvent> myNoteEventListOfLoop(int mode, int template, int chords, int basics[],
			int tensions[], int roots[], int basses[], int lengths[]) {
		ArrayList<MyNoteEvent> res = new ArrayList<MyNoteEvent>();
		int time = 0;
		if (mode == 1) {
			for (int i = 0; i < chords; i++) {
				int[] notes = Chord.notesOfChordWithPianoBasement(basics[i], tensions[i], roots[i], basses[i],
						Chord.pianoBasement);
			...
			for (int i = 0; i < chords; i++) {
				int[] notes = Chord.notesOfChordWithGuitarBasement(basics[i], tensions[i], roots[i], basses[i],
						Chord.guitarBasement + 40);
				for (int j = 0; j < notes.length; j++) {
					MyNoteEvent evt = new MyNoteEvent(true, notes[j], Chord.velocity, time);
					res.add(evt);
				}
	...
	public static Sequence sequenceOfLoop(int mode, int template, int chords, int basics[], int tensions[], int roots[],
			int basses[], int lengths[], int beats, int bar) {
		try {
			...
			ArrayList<MyNoteEvent> events = myNoteEventListOfLoop(mode, template, chords, basics, tensions, roots,
					basses, lengths);
			long time = 0;
			long last = 0;
