...
public class ChordPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	ChordPanel(LoopMaker sv, chordplus rv) {
		...
		/*
		 * cHorizon=new BarCanvas(Color.lightGray); cHorizon.setBounds(16,32,512,1);
		 * add(cHorizon);
		 */
	...
	public void receiveChords(int n, int ds[], int bs[], int ts[], int bass[]) {
		for (int i = 0; i < 12; i++) {
			if (i < n) {
				lChords[i].setText(Chord.chordName((ds[i] + Chord.tonic + 36) % 12, bs[i], ts[i],
						(bass[i] + Chord.tonic + 36) % 12, 0));
	...
	public void startPlaying() {
		synchronized (this) {
			events = Loop.myNoteEventListOfLoop(superview.cTemplateKind.getSelectedIndex() + 1, 0, chords, basics,
					tensions, roots, basses, lengths);
			startTime = System.currentTimeMillis();
			playingPosition = 0;
			playedIndex = 0;
